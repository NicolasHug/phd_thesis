...
reprensational purposes, because it means that we do not have to care about the
...
for representational purposes. Out of the $8$ remaining proportions, $4$
...
\begin{testexample}
  ...
  ways. Also, we \textbf{cannot} say that a calf $(a)$ is to a cow $(b)$ as a
  mare $(d)$ is to a foal $(c)$ (second equivalence class), because such a
  statement would implicitely express that \textit{a young is to an adult as an adult is to
  a young}, which seems wrong: we cannot go from a \textit{young} to an
  \textit{adult} in the same way as we go from an \textit{adult} to
  \textit{young}.  We \textbf{cannot} say either that a calf $(a)$ is to a foal
  $(c)$ as a  mare $(d)$  is to a cow $(b)$ (third equivalence class), because
  this would suggest that \textit{a bovid is to an equid as an equid is to a
  bovid}: here again, we cannot transform the concept \textit{bovid} into
  \textit{equid} in the same way as we would transform the concept
  \textit{equid} into \textit{bovid}.
...
contributions to the task of classification.
