...
In fact, when $\mathbb{R}$ is equiped with the multiplication $\times$, the
couple $(\mathbb{R}, \times)$ is a group (i.e. a set equiped with an operation
...
analogies between sets wlil indeed allow us to derive the definition of an
...
\begin{definition}[Proportion between sets]
...
\begin{definition}[Boolean proportion]
...
Also, note that in fact the $6$ valid patterns of Table
...
$\mathbb{B}$, but it does not really matter: we just have to temporarly
...
is often observed that using the modeling of Klein leads to less accuracte
...
the fact that when the set $B = \Set{0, 1}$ is equiped with the XOR operator,
we obtain a group.
...
Before detailing futher to the geometric insights, we will first make a quick
...
We place ourself here in a very general setting, and consider four elements of
...
\begin{itemize}
  \item The proportion $\mathbf{a}: \mathbf{b} :: \mathbf{c} : \mathbf{d}$ and
    ...
    are distinc, the equivalence class is composed of $8$ elements).
...
principle}, and the process of \textbf{analogical equation solving}.

To this end, we will concentrate on a toy classification problem, in a Boolean
setting. dire que classif important car c'est tout ce qu'on va faire
