...
\begin{testexample}
  ...
  \textit{adult} as we go from an \textit{adult} to \textit{young}.  We
  \textbf{cannot} say either that a calf $(a)$ is to a foal $(c)$ as a  mare
  $(d)$  is to a cow $(b)$ (third equivalence class), because this would
  suggest that \textit{a bovid is to an equid as an equid is to a bovid}: here
  again, we cannot transform the concept \textit{bovid} into \textit{equid} as
  we would transform the concept \textit{equid} into \textit{bovid}.
