...
\ref{SEC:analogical_reco_basic_algo}, we describe our first approach which is
...
Obviously, the five folds are the same for all of the algorithms, to allow for
...
the five folds. All our experiments have been carried out using Surprise
...
\begin{itemize}
  \item The first one is to randomly sample a $3$-tuple in $U_i^3$ $n$ times.
    ...
    \mid^3$ is usually much larger than 1000, so we only have very rough
    estimation here.
...
which assesses the least surprising recommendation (average over all users), we
...
satisfy this criterion, and computing an aggregation of their rating for $i$,
...
approach is denoted as \textit{Rankin's}.
...
first one is obviously impossible in practice due to the terrific size of $2^I$,
...
from the \textit{so bad it's good} kind.
...
binary or numerical do not change anything. The fact that the best proportions
...
where carried out using Surprise\footnote{\url{http://surpriselib.com}}
...
\begin{itemize}
  ...
  \item Various ready-to-use prediction algorithms are available, such as
    baseline algorithms, neighborhood methods, matrix factorization-based (SVD,
