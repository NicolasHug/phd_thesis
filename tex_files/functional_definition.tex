...
classifiers}. These works extend some previous proposal on learning by analogy
in the setting of natural language processing \cite{PirYvo99}. Another form of
analogical classifiers has later been proposed in the works of Bayoudh, Delhay
and Miclet \cite{MicBayDelJAIR08, BayMicDelIJCAI07}, referred-to here as
\textbf{extended classifiers}.
...
\label{ALGO:conservative_classifier}
  \begin{algorithmic}
    ...
    $f(\mathbf{x})$.
    \STATE {\bf Init}: $\mathbf{C}(\mathbf{x}) \leftarrow \varnothing$ \quad \quad // A multiset of candidate
    labels.
...
instance of analogical learner that has ever been proposed, following ideas
that can be already found in \cite{PirYvo99}. We note however that the authors
did not explicitly state that the class equations must be solvable to be
useful.
...
\begin{definition}[Analogical dissimilarity for Boolean values]
...
\begin{algorithm}[!ht]
 \caption{The extended classifier.}
      ...
      \STATE {\bf Input}: A training set $S$, an element $\mathbf{x} \in X^m$
         ...
         \STATE {\bf Output}: $\hat{f}(\mathbf{x})$, an estimation of
         $f(\mathbf{x})$.
    \STATE {\bf Init}: $\mathbf{C}(\mathbf{x}) \leftarrow \varnothing$ \quad \quad // A
...
This fact obviously has a consequence on the way we can work to improve
...
\input{tex_files/functional_definition_fr.tex}
