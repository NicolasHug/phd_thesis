...
\initial{I}n Section \ref{SEC:machine_learning_with_boolean_proportions}, we briefly
...
$\left(\mathbf{x}, f(\mathbf{x})\right)$, called the \textbf{training set} or
also the \textbf{sample set}:
...
\label{ALGO:conservative_classifier}
  \begin{algorithmic}
    \STATE {\bf Input}: A sample set $S$ and an element $\mathbf{x} \in X^m
...
Section \ref{SEC:machine_learning_with_boolean_proportions}, but Algorithm
\ref{ALGO:conservative_classifier} is slightly more general: it allows to deal
with cases where some of the candidate solutions $y$ do not agree with each
other.  The fix here is to consider that the prediction $\hat{f}(\mathbf{x})$
will be the most common candidate solution among all the predictors. This
definition of analogical classification comes from the work of Stroppa and Yvon
\cite{StrYvoCNLL05, StrYvoREPORT05}, and to our knowledge it is the first one
that has every been proposed. We note however that the authors did not
explicitely state that the class equations must be solvable to be useful.

Note that if the multiset $C$ is empty, i.e. if we cannot find
...
\textbf{lazy learners}, i.e. learners that only rely on the sample set $S$
...
crucial concepts: the \textbf{analogical extension} of a sample set $S$, and
...
\begin{definition}
  ...
  Denoting $S$ a sample set and $f$ the ground truth function of the labels,
...
\begin{definition}
  ...
  Given a sample set $S$ with an underlying ground truth function $f$,
...
all elements of the universe $X^m$ are affected by this generilzation.
...
there is no solution for $x=(absurd,G,y)$ just because $R_S^Y(x)= \varnothing$.
...
{\color{red} illustration de minimisation de AD: faire un shema comme Figure
