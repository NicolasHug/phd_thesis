...
It is fairly easy to see that $f_1$ and $f_2$ split the universe $x^m$ into
two disjoint sets of size $2^{m - 1}$: those for which $f_(\mathbf{x})$ is
...
systematically assign the wrong label to   $(1, 1, \cdots 1)$. Therefore, we
will here focus on functions for which datasets are as balanced as possible.
...
\caption{Accuracies of the 1-NAN and 1-NN algorithms  over different Boolean
  functions ($m = 8$) and training set sizes, with corresponding values of
  $\omega$, $\gamma$, and theoretical accuracy. The $x$ axis corresponds to the
...
but wrong one these kind of functions. In chapter \ref{TODO}, we will prove a
...
report the results of the Analogical Proportion Classifier (\textit{APC}) from
