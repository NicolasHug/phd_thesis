...
last, vertice $\mathbf{h}$ will suffer the same fate because the class equation
...
\mathbf{a})}$, and $\aroot{S}{f}{\mathbf{h}} = \varnothing$. It is clear that
in general,
...
\Set{1}$. Both $\albl{\mathbf{g}}$ and $\albl{\mathbf{h}}$ stay undefined.
...
proportion in $\mathbb{R}^m$, the analogical dissmilarity  $\AD(\mathbf{a},
...
\begin{definition}
  ...
  $\mathbb{R}^m$ equiped with the standard $p$-norm $\norm{p}{\cdot}$, the
...
\mathbf{b},\mathbf{c},\mathbf{d}$ in $\mathbb{B}$, $\AD(\mathbf{a},
...
is not closed under addition (or substraction), so technically it may happend that
...
For the sake of simplicity, we have here ignored a small but relevent detail:
...
they will acually consider all the $3$-tuples that have an $\AD$ less than or
...
$3$-tuple is $(\mathbf{a}, \mathbf{b}, \mathbf{e})$ and leads to the prefiction
...
is due to the fact that we (inocently?) chose $k = 1$.}.
...
\mathbf{a}, \mathbf{g}) = 1$, as can be verified on table \ref{TAB:AD_bdag}. We thus have $\mathbf{C}(\mathbf{g}) = \Set{1,
1}$: one solution comes from $f(\mathbf{e)} :f(\mathbf{e}):: f(\mathbf{e}) : y$
and another solution that comes from $f(\mathbf{b)} :f(\mathbf{d})::
f(\mathbf{a}) : y$.  Finally, we have $\hat{f}(\mathbf{g}) = 1$. As for
$\mathbf{h}$, we have $\mathbf{C}(\mathbf{h}) = \Set{1, 1}$: one solution comes
from $f(\mathbf{a)} :f(\mathbf{b}):: f(\mathbf{e}) : y$ and the other comes
from $f(\mathbf{d)} :f(\mathbf{d}):: f(\mathbf{d}) : y$, and we end up with
$\hat{f}(\mathbf{h}) = 1$. This completes our process overview for Algorithm
...
\begin{table}[t]
  ...
  \begin{array}{|cccc|c|}
    \toprule
    \mathbf{b} & \mathbf{d} & \mathbf{a} & \mathbf{g} &  \AD(b_i, d_i, c_i, g_i)\\
    \midrule
    0 & 0 & 0 & 1 &   \textbf{1}\\
    0 & 1 & 0 & 1 &   \textbf{0}\\
    1 & 1 & 0 & 0 &   \textbf{0}\\
...
the next subection is to give a functional translation of this algorithmic
description.
...
We now can reintepret this statement: $\AD(\mathbf{b}, \mathbf{d}, \mathbf{a},
...
\ref{ALGO:extended_classifier} tries to minimise
...
learners. This is the purpose of the next section. Just like for the $k$-NN
litterature, theoretical results are easier to derive when $k = 1$. We will
therefore mostly focus on the properties of the 1-NAN classifier.

\section{Some properties in the real case}
\label{SEC:properties_in_the_real_case}
...
\begin{proof}
  ...
  towards $\mathbf{x}$, Q.E.D.\todo{Lire le paper}
\end{proof}
...
\item The result of Cover and Hart was actually proved in a less restrictive
  ...
  \item We have to be careful about the interpretation of Property
    ...
    property when we are dealing with a classifiaction task. Sadly, such a
...
interesting and poweful theoretical result of analogical classifiers will be
...
defined as:
$$\acc(\NAN_S, X)\eqdef P\left(\hat{f}(\mathbf{x})=f(\mathbf{x}) \given[\big]
...
\begin{itemize}
\item $A \eqdef \{\mathbf{x} \in X^m, \nan(\mathbf{x}, S) \in S\}$: the
  elements that have their nan in $S$.
\item $B \eqdef \{\mathbf{x} \in X^m, \nan(\mathbf{x}, S) \in \esfs\}$: the
  elements that have their nan in $\esfs$.
\end{itemize}

Naturally, $A \cup B = X^m$ and $A \cap B = \varnothing$. Also, $\alpha =
P(\mathbf{x} \in A)$ and $1 - \alpha = P(\mathbf{x} \in B)$. Therefore, the
accuracy of $\NAN_S$ over $X^m$ can be understood as the weighted sum of the
...
of the analogical classifier developed in Section \ref{SEC:functional_definition}: we
