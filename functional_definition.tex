...
Cartesian product of dimension $m$: $X = X_1 \times \ldots \times X_m$. For all
elements $\mathbf{x}$ in a subset $S \subsetneq X$, the class (or label) of
$\mathbf{x}$ is known and is denoted $\dot{\mathbf{x}$ or also somtimes
$f(\mathbf{x})$.

In the context of
classification, items are represented as elements of a universe $X$ having a
(unique) label belonging to $Y$. For any $x\in X$,
$\dot{x}$ denotes the ground truth label associated to $x$. The goal of a
classifier is, given a sample set $S$ (i.e. a set of elements $x \in X$ for
which $\dot{x}$ is known), to correctly predict the label of other elements $x$
that do not belong to the sample set. We call $\hat{x}$ the predicted label of
$x$: this is the output of the classifier.
