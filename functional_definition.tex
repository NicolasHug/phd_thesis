...
use the $1$-norm $\norm{1}{\cdot}$ defined as $\norm{1}{\mathbf{x}} = \sum_{i =
1}^m
|x_i|$, Definition \ref{DEF:AD_arithmetic_proportion} is equivalent to
Definition \ref{DEF:AD_boolean_proportion}. This means that for $\mathbf{a}, \mathbf{b},\mathbf{c},\mathbf{d}$
in $\mathbb{B}$, $\AD(\mathbf{a}, \mathbf{b},\mathbf{c},\mathbf{d}) =
...
$(\mathbf{a}-\mathbf{b})$ and $(\mathbf{c}-\mathbf{d})$.  Here again, we are
...
pure proportions, we allow for some analogies not to be perfect when we need to.
...
they will acually consider all the $3$-tuples that have an $\AD$ lesser than or
...
extended classifier is a generalization of the conservative one and can predict
the output of any $\mathbf{x} \in X^m$.
...
off-the-shelf standard classifiers.

Illustrer l'algo avec a0 b0 d1 et e1.
