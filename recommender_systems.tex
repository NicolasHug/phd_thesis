...
\ref{SEC:Recommender_system_evaluation}).
...
neighborhood approach and matrix factorization-based methods.
...
recommendation example, an item constraint could for example state that a
...
In this section, we formally define the problem of recommendation that we plan
to address later in Chapter \ref{CHAP:analogical_recommendation}. We also
describe how to assess the quality of a recommender system.
...
\begin{itemize}
  ...
  \item Sometimes, the rating scale can be considered as \textbf{ordinal},
    ...
    meaningful when compared to others, i.g. $[$\textit{reject, indifference,
    agreement}$]$.
\end{itemize}
...
Finally, the last option available to evaluate recommender systems is to
...
predictor that has no recommendation power still can outperform the RMSE of
