...
They do so by providing the user with a list of recommendations.
...
of recommendation. If they all share the ultimate goal of providing user with
relevant recommendation, they greatly differ in nature by the items that are
...
are not that sharp, and practical recommendation solutions usually fall in more
than one category. We refer to such systems as \textbf{hybrid} recommenders.
...
name \textit{content}-based), for example movies genre, main actors, film
director, etc\dots.
...
The main advantage of collaborative filtering methods is also its main
...
filtering) are only suitable for certain kinds of items. While movies, books,
or music can all fit quite well within these two framework, this is not the
case for some other items. Take for example the problem of car recommendation.

a content based would fail because too old info and collaborative filtering
impossible BLABLABALABL
