...
\end{proposition}
\noindent
The analogy $A^m$ is thus defined in a component-wise fashion. More often than
not, $A^m$ will be denoted $A$ for the sake of brevity.
...
Note that the geometrical proportion is not the only proportion that deals with
numbers! Indeed, in sections \ref{LesSectnios} we will make use of the
\textbf{arithmetic} proportion, which states an equality of
\textbf{differences}:
\begin{definition}
...
\textcolor{red}{parler des parallélogrammes. Fiare un petit dessin. Faire le
lien avec Rumelhart.}
...
\ref{DEF:analogy_set_facto}. The formula of Definition
...
sufficient one: $a \times d = b\times c$ is satisfied for the pattern $0: 0: 0:
1$, but it is not a valid analogy. Just like in $\mathbb{R}^m$ the arithmetic
proportion allows us to think of Boolean proportions as parallelograms, but
this time in $\mathbb{B}^m$.

\textcolor{red}{mettre des plots de cubes dans B3}
