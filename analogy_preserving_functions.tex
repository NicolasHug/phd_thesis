...
Note though that just like the algorithmic descriptio nof the extended
...
\cite{BayMouMicAnqECML07} never explicitely refer to the analogical extension
...
later in \cite{HugPraRicSerECAI16}, as desribed in Chapter
...
dissimilarity like in  \cite{BayMouMicAnqECML07}, we wille use our equivalent
...
We will show in this section that the classof AP functions is the class of
...
functions that ensure a sound analogical extension of any sample set $S \subseteq
...
Therefore, a more explicit writing of the analogical inference priniple is as
follows: $$
...
This one is the inference prinicple that is actually useful in practice: if the
...
principle B to be sound.  For the sake of completness however, we will also
...
\begin{proof}
  ...
  The four-tuple is summuraized in Table \ref{TAB:SAP}, where we clearly see
...
\subsection{Extension to nominam attributes}
...
We have alread seen in Chapter \ref{CHAP:formal_analogical_proportions} that we
...
is not valid. We can easily extend componentwise this analogy to a Cartesian
...
simpyl because we do not even know how to \textit{write} them. When dealing
with Boolean functions we used an algebraic poitn of view that helped us
...
there are no algebraic structure on the Cartisian product that is the domain of
our functions.
...
where $1$ is at potision $i$.  So if we have a set of nominal attributes $x_1,
...
developped in Section \ref{TODO}. Unfortunately, the whole set $\prod_{i=1}^m
...
P_{\mathbf{x}\in \mathbf{E}^*_S(f)}\left[\albl{\mathbf{x}} =
