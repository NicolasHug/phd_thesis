...
\author{Nicolas Hug\institute{~~IRIT Univ. P. Sabatier, Toulouse, France, email:
name.surname@irit.fr} \and Henri
Prade$^{1,}$\institute{~~QCIS, University of Technology, Sydney, Australia}
\and Gilles  Richard$^{1,}$\institute{~~BITE, London, UK} \and Mathieu
Serrurier$^1$}
%\institute{~~IRIT Toulouse - France, \& QCIS, University of Technology,
%Sydney - Australia} \and Gilles Richard \institute{~~IRIT
%Toulouse - France,  \& BITE, London - UK}\and Mathieu
%Serrurier\institute{~~IRIT Toulouse - France}~ \institute{~~test}}
%email:{\{nhug, prade, richard,email: somename@cs.biu.ac.il}
%serrurier\}}@irit.fr
...
empirical viewpoint.
%as well as finding outsider \cite{PraRicEUSFLAT2013} among a set of examples.
...
%and can be considered as a "common sense" rule.  In this paper, we analyse the
%predictive accuracy for such learners, both strict and relaxed. In order to
%validate this %analysis, we have also plotted the predictive curves and
%compare with artificial benchmarks.

%in light of the results obtained a long time ago regarding the theoretical
%properties of $k$-nn rule \cite{CovHar1967}, we look at some analytic
%properties which could be relevant for analogy-based classifiers.

%On another side, the machine learning community has developed diverse metrics
%to evaluate the capacity (or complexity) of a model class. One of them is the
%so-called {\it VC-dimension}.  This is an integer whose computation is based
%on the discrimination power of the model class.  It is often quite tough to
%compute the VC-dimension of a given class, except for simple cases like
%hyperplanes or simple geometric figures (rectangles, circles, triangles,
...
%Obviously, the trivial proportion
%where $a:b::c:d$ iff $a=b=c=d$ is an analogy as well as the full proportion
%$\forall a, b, c, d, a:b::c:d$.

%Thanks to axioms 2) and 3), it is easy to show that $a:b::c:d$ is equivalent to
%7 other proportions where we permute some elements among $a,b,c,d$. As we have
%24 such permutations, it means we have 3 classes of 8 equivalent proportions.
%For instance, $a:b:d:c$ is equivalent to $d:c::a:b$ but is not equivalent to
%$a:b::c:d$.
...
%Due to the permutation properties of $A$, this is equivalent to 7 other
%equations where the position of $x$ varies. But this is not equivalent to
%$a:b::x:d=1$ or $b:a::c:x=1$ for instance.
...
\path(0,0)node(A){${\bf b}$}--(1,-0.2)node(B){${\bf c}$}--(1,0.8)node(C){\ ${\bf x}$}
--(0,-1)node(D){$\ {\bf a}$}--(1,-1.2)node(E){${\bf
x}'$}--(-1,-0.8)node(F){${\bf x}''$};
...
\draw [line width=1.2pt] [->](B)--(C);
\draw [line width=1.2pt] [->](D)--(A);
%\draw  [line width=1.2pt][densely dotted](B)--(D);
\draw [line width=1.2pt] [->](D)--(B);
%\draw [->] (D)--(E);
\draw [densely dotted](D)--(E);
%\draw [densely dotted][<-] (D)--(F);
\draw [densely dotted](D)--(F);
\draw [densely dotted](A)--(F);
%\draw [<-](B)--(E);
\draw [densely dotted](B)--(E);
\end{tikzpicture}
...
\label{extension}
\begin{center}
\includegraphics[scale=0.20]{pics/dessin.pdf}
\end{center}
\end{figure}
...
\begin{proper}\label{propconvergence}
...
the training set.}
\label{plots}
%\includegraphics[scale=0.35]{zob.pdf}
\includegraphics[width=\linewidth ]{zob.pdf}
\end{figure}
...
\begin{figure}
  ...
  $\mathbb{B}^{10}$.}
\label{omegaplots}
\includegraphics[width=\linewidth]{zob2.pdf}
\end{figure}
