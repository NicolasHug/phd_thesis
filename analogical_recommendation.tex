...
\initial{I}n this chapter, we present our contribution to analogical
...
motivation and as a purpose for analogical proportion mining.
...
algorithm to the Movielens database, and see how this relates to our
...
proportions in a straightforward manner.

Because our method for mining analogical proportion is inspired from the mining
of association rules, we first review this topic in the next section.
...
which would be by the way quite arbitrary. Instead, we will only be interested
...
popular) or $r:r::r':r'$. In the two cases, a switch between $i_3$ and $i_4$ has
...
like $r:r::r:r$. This tells us that in fact, there are simply not a lot of
interesting analogies: the best proportion that we found is
\textit{interesting} on less than 30\% of its components, which is quite small.
This confirms our previous observation that sadly, most of the analogies we
find tend to relate four movies that are actual \textbf{neighbors}.
...
Section \ref{TODO} described our first attempt at building an analogy-based
...
the extended classifier that we have described in Chapter \ref{TODO}. The basic
...
to neighborhood methods, but is extremely expensive in computational resources.
...
recommendation. Acknowledging the fact that each user tends to have its very
own idea of what a good (or bad) rating, we introduced the notion of clones
which captures how two users tend to agree semantically on a rating. This
semantic agreement is not necessarily reflected in the ratings: for $u$ 3 may
...
higher ratings than other, and some items tend to be rated higher than some
other items. We discussed how this view (using biases) related to our view
...
as purely ordinal. This topic did not produced promising results.

In Section \ref{TODO}, we proposed an algorithm for the mining of analogical
...
dataset, such as the one we used in the two previous sections. Based on the
Apriori algorithm, our method is inspired from the mining of association rules
in databases and allows to extract the potential analogies in an efficient way.
...
to the Analogical Dissimilarity as defined in Chapter \ref{TODO}. We gave
...
Boolean values. The main take-away message was that the best analogies that we
found between items related users with similar tastes: the four movies of a
proportion were mostly neighbors, in the sense that users tend to agree on
their respective ratings. Also, the quality of the proportions were there was a
disagreement in the ratings were pretty bad.
...
approach. This is entirely dependent on the database that was used (in our
case, the Movielens dataset), and we may still presume that there should exist
a dataset were the use of analogical proportion is beneficial over a simple
